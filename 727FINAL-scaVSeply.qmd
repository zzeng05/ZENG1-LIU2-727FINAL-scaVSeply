...
#a function to fetch any sca table from data site
...
```{r}
#fetch & clean table25: current business conditions compared with a year ago
BZC <- get_sca_table(25) %>%
...
BZC_supp <- BZC %>%
  transmute(
    date,
    better_now = `Better Now`,
    same_now   = Same,
    worse_now  = `Worse Now`,
    dk_now     = `DK; NA`,
    net_biz    = better_now - worse_now  # >0: net positive view of business conditions
  )

head(BZC_supp)

BZC_long <- BZC_supp %>%
  select(date, better_now, same_now, worse_now) %>%
  pivot_longer(
    cols = -date,
    names_to  = "response",
    values_to = "share"
...
ggplot(BZC_long, aes(x = date, y = share, fill = response)) +
  geom_area(alpha = 0.8) +
  scale_fill_manual(
    values = c(
      better_now = "darkgreen",
      same_now   = "grey70",
      worse_now  = "firebrick"
    ),
    labels = c("Better Now", "Same", "Worse Now")
  ) +
  labs(
    title = "Current Business Conditions Compared with a Year Ago",
    x     = "Date",
...
plot_cs_vs_netbiz_dual <- function(data_macro, data_bzc,
                                   cs_limits  = c(40, 120),
                                   net_limits = c(-90, 30)) {
  df <- data_macro %>%
    select(date, cs) %>%
    left_join(data_bzc %>% select(date, net_biz), by = "date") %>%
    filter(!is.na(cs), !is.na(net_biz)) %>%
    arrange(date)
  ...
  df <- df %>%
    mutate(
      net_biz_scaled = cs_min + (net_biz - nb_min) * (cs_rng / nb_rng)
    )
  
  ggplot(df, aes(x = date)) +
    geom_line(aes(y = cs, color = "Consumer Sentiment")) +
    geom_line(aes(y = net_biz_scaled, color = "Net business conditions (Better-Worse)")) +
    scale_y_continuous(
      name   = "Consumer Sentiment Index",
      limits = cs_limits,
      sec.axis = sec_axis(
        trans = ~ (.- cs_min) * (nb_rng / cs_rng) + nb_min,
        name  = "Net % saying business conditions are better minus worse"
      )
    ) +
    scale_color_manual(
      values = setNames(
        c("steelblue", "darkorange"),
        c("Consumer Sentiment", "Net business conditions (Better-Worse)")
      )
    ) +
    labs(
      title = "Headline sentiment vs. net views of business conditions",
      x     = "Date",
      color = NULL
    ) +
    theme_minimal() +
...
p_netbiz <- plot_cs_vs_netbiz_dual(macro, BZC_supp,
                                   cs_limits  = c(40, 120),
                                   net_limits = c(-90, 30))
...
cs_vs_unemp (macro, lag_months = 3)
```
```{r}
#| echo: false
#| warning: false
...
cs_vs_jobs <- function(data,
                                 ...
                                 jobs_limits = c(-1000, 1000)) { # right y-axis (thousands)
  ...
  ggplot(df, aes(x = date)) +
    ...
    scale_y_continuous(
      ...
      sec.axis = sec_axis(
        ...
        name  = paste0("Monthly change in nonfarm employment (thousands) at t + ",
                       lag_months, " months")
      )
    ) +
    scale_color_manual(
...
## Figure 2. Scatter: net expectations vs subsequent unemployment change
```{r}
#| echo: false
#| warning: false
macro2 <- macro %>%
  ...
  arrange(date) %>%
  mutate(
    unrate_3m_delta = lead(unrate, 3) - unrate
  )
df_scatter <- macro2 %>%
...
## Figure 3. Time series: net expectations vs actual unemployment (dual axis)
```{r}
plot_expect_vs_unemp_dual <- function(data_macro2,
  ...
  ggplot(df, aes(x = date)) +
    ...
    geom_line(aes(y = unrate_scaled,    color = "Unemployment rate")) +
    scale_y_continuous(
      name   = "Net % expecting less minus more unemployment",
      limits = cs_limits,
      sec.axis = sec_axis(
    ...
    scale_color_manual(
      values = setNames(
        ...
        c("Net expectations (Less-More)", "Unemployment rate")
      )
    ) +
    labs(
      title = "Net expectations about unemployment vs actual unemployment",
      x     = "Date",
      color = NULL
    ) +
    theme_minimal() +
...
p_expect_unemp <- plot_expect_vs_unemp_dual(macro2,
                                            cs_limits    = c(-60, 60),
                                            unemp_limits = c(2, 12))
p_expect_unemp
```
